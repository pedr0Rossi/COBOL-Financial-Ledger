       ...
       01  WS-CHANGE-ID              PIC 9(5).
       01  WS-NEW-DESCRIPTION        PIC X(20).
           ...
           PERFORM UNTIL EOF OR FOUND
               READ TRANSACTIONS NEXT RECORD
                   AT END
                       SET EOF TO TRUE
                   NOT AT END
                       PERFORM 0200-CHANGE-DESCRIPTION
               END-READ
           END-PERFORM                       
       ...
       0150-PROMPT-USER.
           DISPLAY "ENTER THE ID OF THE TRANSACTION YOU WISH TO CHANGE:"
           ACCEPT WS-CHANGE-ID
       ...
       0200-CHANGE-DESCRIPTION.
           IF TR-ID = WS-CHANGE-ID AND NOT-FOUND
              SET FOUND TO TRUE

              IF TR-STATUS = "CANCELED"
                 DISPLAY "TRANSACTION IS NOT ACTIVE. NO CHANGES MADE."
                 PERFORM 0900-END-PROGRAM
              END-IF

              MOVE WS-NEW-DESCRIPTION TO TR-DESC
              REWRITE FD-TRANSACTION-DETAILS
              DISPLAY "DESCRIPTION CHANGED SUCCESSFULLY!"
           END-IF.
       0200-END.
